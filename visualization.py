...
from datasetUtils import load_dataset, get_dataset_samples_and_statistics

from DCNNs import validate
from termcolor import colored


def main(gpu_ids, uccs_cluster, target, model_path, dir_to_save_images):
	...
	if len(queries_images_target) == 1:
		queries_images_target = queries_images_target[0]
	
	if len(gallery_images_target) == 1:
		gallery_images_target = gallery_images_target[0]

	if queries_images_target[0][3] == 'person':
		img_height = 256
		img_width = 128
	elif queries_images_target[0][3] == 'object':
		img_height = 224
		img_width = 224
	else:
		print("%s is a kind of ReID not implemented" % queries_images_target[0][3])
	...
	h_image = img_height + 20 # Original height  + 10 colored pixels upper and lower + 10 white pixels upper and lower
	w_image = img_width + 20 # Original height + 10 colored pixels on right and left + 10 white pixels on right and left

	visualization = ReIDVisualization(bn_mean.cpu(), bn_var.cpu(), bn_weights.cpu(), bn_bias.cpu(), img_height, img_width)
	...
	for key in query_keys:
		...
		print("ID:", queries_images_target[query_idx][0], queries_images_target[query_idx][1])
		imgPIL_query = torchreid.utils.tools.read_image(queries_images_target[query_idx][0])
		imgPIL_query = Resize((img_height,img_width))(imgPIL_query)
		...
		for gal_idx in gallery_indexes:
			gal_ids.append([gallery_images_target[gal_idx][0], gallery_images_target[gal_idx][1]])
			imgPIL_gallery = torchreid.utils.tools.read_image(gallery_images_target[gal_idx][0])
			imgPIL_gallery = Resize((img_height,img_width))(imgPIL_gallery)
...
class ReIDVisualization(object):
    ...
    def getHeatmap(self, FeatureMap_i, FeatureMap_j, fv_i, fv_j):
        ...
        heatmap = Resize((self.height,self.width))(ToPILImage()(heatmap))

        return heatmap

    def blendImageHeatmap(self, img, heatmap, alpha=0.5):
...
if __name__ == '__main__':
	...
	parser.add_argument('--uccs_cluster', action="store_true", help='Define if its running in the uccs or Unicamp cluster')
	...
	main(gpu_ids, uccs_cluster, target, model_path, dir_to_save_images)